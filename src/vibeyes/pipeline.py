"""Gaze pipeline shared between autoresearch and the live system.

The autoresearch agent modifies this file to improve accuracy.
The live system (detector.py) wraps these functions for real-time use.

Public API:
  - create_landmarker(model_path) -> FaceLandmarker
  - extract_gaze_features(frame, landmarker) -> tuple | None
  - is_blink(features) -> bool
  - fit_calibration(gaze_points, screen_points) -> (coeffs_x, coeffs_y, norm_x, norm_y)
  - calibration_predict(features, coeffs_x, coeffs_y, norm_x, norm_y) -> (x, y)
  - replay_calibration(frames_dir, calibration_clicks) -> PipelineState
...
def create_landmarker(model_path: str | None = None) -> FaceLandmarker:
...
def extract_gaze_features(frame: np.ndarray, landmarker: FaceLandmarker) -> tuple[float, ...] | None:
    """Extract 17 gaze features from a BGR frame.

    Returns None if no face detected. Feature indices:
      0: left eye x ratio       1: left eye y ratio
      2: right eye x ratio      3: right eye y ratio
      4: head yaw               5: head pitch
      6: inter-pupillary dist   7: left EAR    8: right EAR
      9: avg iris abs x        10: avg iris abs y
     11: nose offset x         12: nose offset y
     13: iris-nose left x      14: iris-nose left y
     15: iris-nose right x     16: iris-nose right y
    """
    h, w = frame.shape[:2]
    rgb = frame[:, :, ::-1]
...
def is_blink(features: tuple[float, ...]) -> bool:
    """Check if features indicate a blink (either eye closed)."""
    left_ear = features[7]
    right_ear = features[8]
    return left_ear < EAR_BLINK_THRESHOLD or right_ear < EAR_BLINK_THRESHOLD
...
# ============================================================
# Public API (called by autoresearch prepare.py)
# ============================================================
